...
function Jenkins(runner, options) {
  ...
  function endSuite() {
    if (currentSuite != null) {
      ...
      try {
      genSuiteReport();
      if (currentSuite.failures > 0) {
        log('**********************');
        log(fd);
        log();
        log(currentSuite.suite.fullTitle());
        log(xml(jsonResults, {indent: '  '}));
        log('**********************');
      }
      } catch (err) { log(err) }
      currentSuite = null;
  ...
  runner.on('end', function() {
    ...
    if (fd) {
      ...
      log('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&');
      log('end was called, but file descriptor is already closed!');
      log(currentSuite);
      log('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&');
    }
